...
import PEAR_two_rm as compress_model
...
# python PEARencodingdic.py --input_dir combined_file --prefix commoe --gpu_id 7 --batch_size 20000
...
def decode(temp_dir, compressed_file, FLAGS, len_series, last):
  ...
  iter_num = (len_series - FLAGS.seq_len) // bs
  ...
  for train_index in range(iter_num-FLAGS.seq_len):
    ...
    logits, _ = model.forward(train_batch, None) 
    prob = logits[:, -1, :]
    ...
    label = torch.from_numpy(series_2d[:, train_index + FLAGS.seq_len]).cuda().long()
  ...
  # --- FIX STARTS HERE ---
  # Open the output file in binary write mode ('wb') to prevent character encoding issues.
  with open('decompressed_out', 'wb') as out:
    # Write the main decoded data streams.
    for i in range(series_2d.shape[0]):
      # Convert the numpy array of integers directly to bytes and write to the file.
      out.write(series_2d[i].astype(np.uint8).tobytes())
  
    # Close the input files for the main data streams.
    for i in range(bs):
      bitin[i].close()
      f[i].close()

    # If there is a final, smaller chunk of data, decode and append it.
    if last:
      series = np.zeros(last, dtype = np.uint8).astype('int')
      f_last = open(temp_dir+"/"+compressed_file+'.last','rb')
      bitin_last = arithmeticcoding_fast3.BitInputStream1(f_last)
      dec_last = arithmeticcoding_fast3.ArithmeticDecoder(32, bitin_last)
      ...
      for j in range(last):
        series[j] = dec_last.read(cumul, FLAGS.vocab_size)
    
      print("Last decode part don't need inference.")
      # Append the final decoded bytes to the same output file.
...
@jit(nopython=True)
def cumulative_sum_inplace(prob):
    prob *= 10000000
    prob += 1
...
def data_handling1(shared_cumul, shared_y, num_processes, chunk_size, bs, FLAGS, queue, shared_queue):
    while True:
        ...
        if bs > 4000:
          ...
          cumul_batch[:, 1:] = cumulative_sum_inplace(prob)
        copy_to_shared_memory(shared_cumul, shared_y, cumul_batch, y, 0, bs)
        queue.put(1)
...
def model_prediction_task(shared_cumul, shared_y, queue, shutdown_flag, FLAGS, series, train_data, chunk_size, num_processes):
    ...
    for train_index in range(iter_num):
        ...
        logits, emb = model.forward(train_batch[:, :-1], y)
        logits = logits.transpose(1, 2)
    ...
    print(f"Model prediction time: {(model_end_time - model_start_time)/3600} h")
    queue.put('exit')
    print("Model prediction process exited")
...
def encode(temp_dir, compressed_file, FLAGS, series, train_data, last_train_data):
    ...
    print(f"Cycle time: {(cycle_end_time - cycle_start_time)/3600} hour")
    ...
    print(f"Total compression time: {total_duration / 3600} hours")

    if last_train_data is not None:
        print("last series")
        f = open(temp_dir + "/" + compressed_file + '.last', 'wb')
        bitout = arithmeticcoding_fast3.BitOutputStream1(f)
        enc = arithmeticcoding_fast3.ArithmeticEncoder(32, bitout)
        prob = np.ones(FLAGS.vocab_size) / FLAGS.vocab_size
        cumul = np.zeros(FLAGS.vocab_size + 1, dtype=np.uint64)
        cumul[1:] = np.cumsum(prob * 10000000 + 1)
    ...
    total_duration = time.time() - start_time
    print(f"Total compression time 2: {total_duration / 3600} hours")
...
def main(_):
  ...
  with open(FLAGS.input_dir, 'rb') as fp:
